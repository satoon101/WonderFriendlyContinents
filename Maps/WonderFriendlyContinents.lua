...
function GenerateMap()
    ...
    -- --- SNOW AND TUNDRA ADJUSTMENT ---
    local iW, iH = Map.GetGridSize()
    for i = 0, (iW * iH) - 1 do
        local pPlot = Map.GetPlotByIndex(i)
        local y = pPlot:GetY()
        local lat = y / iH -- 0.0 at south, 1.0 at north

        -- 1. Reduce Tundra: Convert edge-case Tundra to Plains
        if (lat > 0.15 and lat < 0.22) or (lat > 0.78 and lat < 0.85) then
            if pPlot:GetTerrainType() == g_TERRAIN_TYPE_TUNDRA then
                TerrainBuilder.SetTerrainType(pPlot, g_TERRAIN_TYPE_PLAINS)
            end
        end

        -- 2. Force Snow: Ensure the very top and bottom rows have land-snow
        if not pPlot:IsWater() then
            if lat < 0.05 or lat > 0.95 then
...
function AddRivers()
    ...
    for iPass, passCondition in ipairs(passConditions) do
        local riverSourceRange = (iPass <= 2) and riverSourceRangeDefault or (riverSourceRangeDefault / 2);
        local seaWaterRange = (iPass <= 2) and seaWaterRangeDefault or (seaWaterRangeDefault / 2);

        local iW, iH = Map.GetGridSize();
        for i = 0, (iW * iH) - 1 do
            local plot = Map.GetPlotByIndex(i);
            if(not plot:IsWater()) then
                if(passCondition(plot) and not plot:IsNaturalWonder() and not AdjacentToNaturalWonder(plot)) then
...
function GeneratePlotTypes()
	...
	local done = false;
	local iAttempts = 0;
	local iWaterThreshold, iNumTotalLandTiles, iNumBiggestAreaTiles, iBiggestID;
	while done == false do
...
function AddFeatures()
	...
	for x = 0, g_iW - 1 do
		for y = 0, g_iH - 1 do
			local lat = math.abs((y / g_iH) - 0.5) * 2.0
			if lat > 0.8 and lat < 0.9 then
				local plot = Map.GetPlot(x, y)
				local feature = plot:GetFeatureType()
				if feature == g_FEATURE_ICE then
					local iRandom = TerrainBuilder.GetRandomNumber(2, "Ice or nah")
					if lat < 0.85 or iRandom == 0 then
...
function GenerateCenterRift(plotTypes)
	...
	local horizontalDrift = 5;
	local verticalDrift = 3;
	--
	if riftLean == 0 then
...
function IsAdjacentToOcean(x, y)
	for direction = 0, 5 do
		...
		if adjacentPlot and adjacentPlot:GetTerrainType() == g_TERRAIN_TYPE_OCEAN then
			return true
		end
	end
	return false
...
function IsSettleableLand(plot)
...
function CreateBridgePoint(x, y)
    local pPlot = Map.GetPlot(x, y)
    local pL1   = Map.GetPlot(x - 1, y) -- One left
    local pL2   = Map.GetPlot(x - 2, y) -- Two left
    local pR1   = Map.GetPlot(x + 1, y) -- One right
    local pR2   = Map.GetPlot(x + 2, y) -- Two right

    -- Safety check: ensure all plots exist (especially near map edges)
    if not (pPlot and pL1 and pL2 and pR1 and pR2) then
		return false
	end

	if pPlot:GetTerrainType() ~= g_TERRAIN_TYPE_COAST then
		return false
	end

	if IsAdjacentToOcean(x, y) then
		return false
	end

	if (
		pL1:GetTerrainType() ~= g_TERRAIN_TYPE_COAST or
		pR1:GetTerrainType() ~= g_TERRAIN_TYPE_COAST
	) then
		return false
	end

	if not (IsSettleableLand(pL2) or IsSettleableLand(pR2)) then
		return false
	end

	-- The random logic
	local terrainChoice = g_TERRAIN_TYPE_PLAINS
	if TerrainBuilder.GetRandomNumber(2, "Plains vs Grass") == 0 then
